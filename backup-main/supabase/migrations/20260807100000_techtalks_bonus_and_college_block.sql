-- 15 regular TechTalks teams (30 seats) may select both categories.
-- The next 5 teams may select TechTalks only.
update public.events set capacity = 40, team_size = 2 where slug = 'techtalks';
...
declare
  ...
  v_tech integer := 0; v_nontech integer := 0; v_techtalk_count integer := 0;
  v_bonus_techtalks boolean := false;
...
begin
  if trim(payload->>'registerNumber') like '8204%' then
    raise exception 'Your register number belongs to our college. Registration is only for other colleges.';
  end if;
  if coalesce(array_length(v_events, 1), 0) not in (1, 2) or (array_length(v_events, 1) = 2 and v_events[1] = v_events[2]) then
  ...
  if (select count(*) from events where slug = any(v_events)) <> array_length(v_events, 1) then raise exception 'One selected event no longer exists.'; end if;
  ...
  foreach v_slug in array v_events loop
    ...
    if v_slug = 'techtalks' then v_techtalk_count := v_count; end if;
    ...
    if v_team_size = 2 then
      ...
      if trim(v_partner->>'registerNumber') like '8204%' then raise exception 'Your teammate''s register number belongs to our college. Registration is only for other colleges.'; end if;
  ...
  if array_length(v_events, 1) = 1 then
    if v_events[1] <> 'techtalks' or v_techtalk_count < 30 then raise exception 'TechTalks-only registration opens after the first 15 TechTalks teams are full.'; end if;
    v_bonus_techtalks := true;
  elsif v_tech <> 1 or v_nontech <> 1 then
    raise exception 'Select exactly one technical event and one non-technical event.';
  elsif 'techtalks' = any(v_events) and v_techtalk_count >= 30 then
    raise exception 'The regular TechTalks teams are full. Select TechTalks only for a bonus team registration.';
  end if;
