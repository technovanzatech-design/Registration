-- TechTalks and Fun Feast can have different teammates for the same participant.
...
declare
  ...
  v_id uuid; v_participant_id text; v_first_partner jsonb; v_partner jsonb;
...
begin
  ...
  foreach v_slug in array v_events loop
    ...
    if v_team_size = 2 then
      ...
      if v_first_partner is not null and (v_partner->>'registerNumber' = v_first_partner->>'registerNumber' or lower(v_partner->>'email') = lower(v_first_partner->>'email') or v_partner->>'phone' = v_first_partner->>'phone') then raise exception 'Use different teammates for TechTalks and Fun Feast.'; end if;
