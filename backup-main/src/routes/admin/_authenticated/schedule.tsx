...
function SchedulePage() {
  ...
  return <div className="flex flex-col gap-6">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h1 className="font-display text-2xl font-bold text-foreground">Conflict-Free Schedule</h1><p className="text-sm text-muted-foreground">Approve up to 13 TechTalks teams, then create two non-overlapping event slots for every participant and teammate.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={refresh}><RefreshCw className="size-4" /> Refresh</Button><Button variant="outline" onClick={exportExcel} disabled={exporting}><FileDown className="size-4" /> {exporting ? "Exporting…" : "Export Excel"}</Button><Button variant="outline" onClick={sendMissingSchedules} disabled={sendingSchedules || !assignments.data?.length}><Mail className="size-4" /> {sendingSchedules ? "Sending…" : "Send Schedule Emails"}</Button><Button onClick={generate} disabled={loading}><ShieldCheck className="size-4" /> Generate schedule</Button></div></div>
    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><UsersRound className="size-4 text-primary" /> TechTalks paper approvals</CardTitle><p className="text-sm text-muted-foreground">Approve up to 13 teams. Each slot can hold up to 7 teams, while the overall TechTalks limit remains 13 teams. Their selected non-technical event is placed in the opposite slot.</p></CardHeader><CardContent>{teams.isLoading ? <p>Loading teams…</p> : <div className="grid gap-3 md:grid-cols-2">{teams.data?.map((team) => <div key={team.team_key} className="flex items-center justify-between rounded-xl border border-border p-3"><div><p className="font-medium">{team.member_one_name ?? team.member_one_register_no} + {team.member_two_name ?? team.member_two_register_no}</p><p className="text-xs text-muted-foreground">{team.member_one_register_no} · {team.member_two_register_no}</p></div><Button size="sm" variant={team.approved ? "default" : "outline"} onClick={() => setTeamApproval(team, !team.approved)}>{team.approved ? "Approved" : "Approve"}</Button></div>)}</div>}</CardContent></Card>
